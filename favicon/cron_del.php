...
$sql = "SELECT `exmname` FROM  `icon` WHERE `FLG` = 'ON' AND `expiredate` <= '$deldate'";
$result = mysql_query($sql, $db_conn);
while($link = mysql_fetch_row($result))
{
	...
	//ディレクトリ削除
	// $delpath = $TMP.$exmname;
	remove_directory("/usr/share/nginx/html/faceapglezon.info/favicon/tmp/".$exmname);
	remove_directory("/usr/share/nginx/html/faceapglezon.info/favicon/uploads/".$exmname);
	//フラグアップデート
