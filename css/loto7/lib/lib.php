...
require_once dirname(__FILE__) . "/lib/qdmail.php";
//Qdsmtpをロード
//（ドキュメントには、記述不要とかいてあるが、書かないとうまくいかないことがあった）
// require_once('qdsmtp.php');
require_once dirname(__FILE__) . "/lib/qdsmtp.php";
...
//_
function f_get_flickr($latest_arr)
{
	$rtn = "";
	//画像のみ
			foreach ($latest_arr as $key => $value)
			{
				if($key==1||$key==2||$key==3||$key==4||$key==5||$key==6)
				{
					$rtn .=  '<script type="text/javascript" src="http://metaatem.net/spell.php?picsize=s&string='.$value.'&height=33&no_link=1"></script>'."_";
				}elseif($key==7)
				{
					$rtn .=  '<script type="text/javascript" src="http://metaatem.net/spell.php?picsize=s&string='.$value.'&height=33&no_link=1"></script>';
				}
			}
}
/////////////////////////
// flickr 数字画像取得
function f_get_number($number)
{
...
loto7
Clave:
87787f8c4d97f554ef80cdd7e82700e1
Secreto:
3bd856fed4ed809b
...
*/
        ...
        // }
// Flickr APIKEY
	define (APIKEY, "87787f8c4d97f554ef80cdd7e82700e1");

// 検索ワード
//$searchWord = "1";	//検索ワード

	// サムネイルサイズ(s.t.m.z.bよりサイズを設定)
	$pictureSize = "s";	//サムネイルサイズ
...
switch ($number) {
	...
	$url = "http://www.flickr.com/services/rest/?"
	     ...
	     . "&format=rest"
	     . "&sort=interestingness-desc"     //人気の高い順に検索'&sort='.$sort.
	     . "&api_key=".APIKEY
	     . "&per_page=100"
	     . "&license=1,2,3,4,5,6"
//	     . "&extras=owner_name"
...
//echo "url=".$url ;
	...
	foreach($xml->photos->photo as $photoValue){
	    ...
	    //
	    $img_url = "http://farm".$farmId.".static.flickr.com/".$serverId."/".$photoId."_".$secret."_s.jpg";
...
// echo "<hr>";
	...
	$rand_keys = array_rand($rtn_array,10);
	$rtn_st = $rtn_array[$rand_keys[0]];
	return $rtn_st;
}
//予測記録
