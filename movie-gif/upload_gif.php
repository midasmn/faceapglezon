...
if(isset($_FILES['userfile']) && $_FILES['userfile']['error'] == 0)
{
  ...
  }else
  {
    ...
    if(move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/'.$exm_dir.'/'.$exm_dir.'.'.$extension))
    {
      ...
      //処理時間をはずす
      // set_time_limit(0);
      $rtn = f_make_gif($WIDTH,$exm_dir,'uploads/'.$exm_dir.'/'.$exm_dir.'.'.$extension);
      flush();
      // //DBに記録
      if($rtn=="OK")
      {
        ...
        // $rtn_time = "12:00";
        $rtn = f_insert_upfile($db_conn,$filename,$exm_dir,$size,$rtn_time,'gif',$user_agent,$expiredate);
      }
      flush();
      //
...
// HTMLの時用
$HTML_BASE = "http://faceapglezon.info/movie-gif/tmp/";
$TMP_PATH ="/usr/share/nginx/html/faceapglezon.info/movie-gif/tmp/"; 
$TMP_HTML_BASE = "http://faceapglezon.info/movie-gif/tmp/";
