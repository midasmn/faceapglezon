...
// ffmpeg convertメイン
//////////////////////
function f_make_gif($WIDTH,$exm_dir,$filepath)
{
	//ファイル名
	...
	$filename = $exm_dir;
	//処理フォルダ
	$TMP="/usr/share/nginx/html/faceapglezon.info/movie-gif/tmp";
...
function f_make_png($WIDTH,$exm_dir,$filepath)
{
  ...
  $filename = $exm_dir;
  //処理フォルダ
  $TMP="/usr/share/nginx/html/faceapglezon.info/tools/mpegmagick/tmp";
  mkdir($TMP.'/'.$exm_dir, 0777);
  $OUT_PATH=$TMP.'/'.$exm_dir;
  //幅取得
  $get_width = f_get_width($filepath);
...
//
function f_sound_only($WIDTH,$exm_dir,$filepath)
{
  //ファイル名
  $path_parts = pathinfo($filepath);
  // $filename = $path_parts['filename'];
  ...
  $OUT_PATH=$TMP.'/'.$exm_dir;
  //音声抜き出し
  system("/usr/bin/ffmpeg  -i ${filepath}  -ab 128   ${OUT_PATH}/{$filename}.mp3");
  flush();
  //存在確認
  if ( file_exists( $OUT_PATH."/".$filename.".mp3")) 
  {
    $rtn = "OK";
  } else {
    $rtn = "NG";
...
{
  $TMP_PATH ="/usr/share/nginx/html/faceapglezon.info/tools/mpegmagick/tmp/";
